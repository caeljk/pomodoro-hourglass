...
// repeat a utf-8 string n times (can't use std::string(n, char) for multi-byte glyphs)
...
// sand cells in bottom-half row r (r=1 near neck, r=HALF at floor) given progress p
...
std::string sand_color(Phase ph) {
    if (ph == Phase::SHORT_BREAK) return ansi::rgb(70,  210, 130);  // mint green
    if (ph == Phase::LONG_BREAK)  return ansi::rgb(80,  160, 240);  // sky blue
    return                               ansi::rgb(220, 140,  40);  // amber (work)
}
...
void draw_clock(std::ostream& out, int secs, const std::string& color, int row0, int col0) {
    int mm = secs / 60;
    int ss = secs % 60;
...
std::string wall_clock() {
    ...
    for (char& c : buf) if (c >= 'A' && c <= 'Z') c = (char)(c + 32);
    return buf;
}
...
void splash(int /*rows*/, int cols) {
    ...
    std::cout << ansi::mv(br,            bc) << dc << "\u250c" << ansi::rep("\u2500", BOX_W - 2) << "\u2510" << ansi::RST;
    for (int r = 1; r < BOX_H - 1; r++) {
        std::cout << ansi::mv(br + r, bc) << dc << "\u2502" << std::string(BOX_W - 2, ' ') << "\u2502" << ansi::RST;
    }
    std::cout << ansi::mv(br + BOX_H - 1, bc) << dc << "\u2514" << ansi::rep("\u2500", BOX_W - 2) << "\u2518" << ansi::RST;

    std::cout << ansi::mv(br + 2, bc + 4) << am << ansi::BOLD << "pomodoro" << ansi::RST;
    std::cout << ansi::mv(br + 3, bc + 4) << gc  << "terminal focus timer  v1.0" << ansi::RST;

    std::cout.flush();

    struct Entry { const char* label; int delay_ms; };
    Entry entries[] = {
    ...
    std::cout << ansi::mv(br + BOX_H - 2, bc + 4) << gc << "press any key to start" << ansi::RST << std::flush;
...
void render(std::ostream& out, const Timer& tmr, int /*rows*/, int cols, int frame) {
    if (cols < 60) {
...

    {
        ...
        out << ansi::mv(1, 2)
            << amber_c << ansi::BOLD << "pomodoro" << ansi::RST
            << gray_c  << "  \xe2\x80\x94  terminal focus timer" << ansi::RST
            << ansi::EL;

        out << ansi::mv(1, wc_col) << gray_c << wc << ansi::RST;
        out << ansi::mv(2, 1) << dim_c << ansi::rep("\u2500", cols - 1) << ansi::RST << ansi::EL;
    }

    hg::draw(out, tmr, hg_row, hg_col, frame);

    out << ansi::mv(hg_row, rp_col) << dim_c << "countdown" << ansi::RST << ansi::EL;
...

    {
        ...
        out << ansi::mv(pr, rp_col) << sand_c << ansi::BOLD << ph << ansi::RST << ansi::EL;
    }
    ...
    {
        int cr = hg_row + 12;
        out << ansi::mv(cr, rp_col) << dim_c << "\u2500 controls" << ansi::RST << ansi::EL;
        ...
        for (int i = 0; i < 4; i++) {
            out << ansi::mv(cr + 1 + i, rp_col)
                << kc << "[" << binds[i].key << "]" << ansi::RST
                << gray_c << "  " << binds[i].desc << ansi::RST << ansi::EL;
        }
    }
    ...
    while (!g_quit && !tmr.quit) {
        ...
        while (term::kbhit()) {
            switch (std::tolower(term::getch())) {
                case ' ': tmr.paused = !tmr.paused; break;
                case 's': tmr.skip();  break;
                case 'r': tmr.reset(); break;
                case 'q': tmr.quit = true; break;
            }
        }
